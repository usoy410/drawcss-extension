...
import { TechStack } from "./scanner.js";
...
export async function generateAgenticCode(drawingData: string, techStack: TechStack): Promise<string> {
    ...
    const apiKey = process.env.GEMINI_API_KEY || vscode.workspace.getConfiguration('drawcss').get<string>('apiKey');
...
function getSystemInstruction(framework: string, styling: string) {
